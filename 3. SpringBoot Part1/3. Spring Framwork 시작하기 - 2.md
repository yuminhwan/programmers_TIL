...
1. 초기화시에 필요한 모든 의존 관계가 형성되기 때문에 안전하다. 
	...
	- 초기화시에 필요한 걸 생성자를 통해 명시 즉, 의존관계를 외부로 노출시켰으니 주입이 필요한 객체는 무조건 주입해야 하기 때문이다.
		- 하지만 주입한 필드가 `null`이 될 수 도 있으니 그럴땐 Optional 타입을 사용한다.
